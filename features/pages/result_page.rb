class ResultPage
  def initialize
    @lbl_search_result=".a-color-state.a-text-bold"
    @lbl_no_search_result=".s-search-results"
  ...
  def verify_search_results(item)
    find(@lbl_search_result).text.should =='"'+item+'"'
  end

  def verify_no_search_result(item)
    find(@lbl_no_search_result).text.should include("No results for " + item)
  end
